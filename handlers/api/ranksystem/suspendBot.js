...
module.exports = async (req, res) => {
    try {
        ...
        //
        try {
            await panel.suspendBot({ templateName: bot.template_name })
            await bot.update({ state: "suspended", status: "offline" })
        } catch (err) {
            return res.status(apiCodes.BAD_REQUEST).json(err.error)
        }

        return res.status(apiCodes.SUCCESS).json(responses.RANKSYSTEM.SUSPENDED)
    } catch (err) {
