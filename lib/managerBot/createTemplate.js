const createTemplate = (templateName, channels, conn) => {
    for (let key of Object.keys(channels)) {
        ...
        if (key == "forbiddenMsgNotify") {
            channels[key].sgListToNotify = ch.sgListToNotify !== "" ? ch.sgListToNotify.split(",") : []
            channels[key].ignoreSgList = ch.ignoreSgList !== "" ? ch.ignoreSgList.split(",") : []
            channels[key].forbiddenWords = ch.forbiddenWords !== "" ? ch.forbiddenWords.split(",") : []
        }

        if (key == "voiceStatusChange") {
            channels[key].sgListToNotify = ch.sgListToNotify !== "" ? ch.sgListToNotify.split(",") : []
            channels[key].ignoreSgList = ch.ignoreSgList !== "" ? ch.ignoreSgList.split(",") : []
            channels[key].adminPokeFormats = JSON.parse(ch?.adminPokeFormats) || []
            channels[key].clientPokeFormats = JSON.parse(ch?.clientPokeFormats) || []
        }
    }
